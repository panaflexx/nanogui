...
Window::Window(Widget *parent, const std::string &title)
    : FramebufferWidget(parent), m_title(title), m_button_panel(nullptr), m_modal(false),
      m_drag(false), m_animate(None) { }
...
void Window::draw(NVGcontext *ctx) {
    ...
    //Color fill = m_mouse_focus ? m_theme->m_window_fill_focused
    //                           : m_theme->m_window_fill_unfocused;

    //printf("Drawing Window %s\n", m_title.c_str());

    if(m_animate == Closing) {
        if(m_fade[3] < 1) {
            m_fade[3] += 0.05;
            printf("m_fade = %f\n", m_fade[3]);
        } else {
    ...
    nvgFillColor(ctx, m_mouse_focus ? Color(m_theme->m_window_fill_focused - m_fade)
                                    : Color(m_theme->m_window_fill_unfocused - m_fade));
    ...
    if (!m_title.empty()) {
        ...
        nvgBeginPath(ctx);
        nvgRoundedRect(ctx, m_pos.x(), m_pos.y(), m_size.x(), hh, cr);
        nvgStrokeColor(ctx, Color(Vector4f(m_theme->m_window_header_sep_top) - Vector4f(m_fade)));
        ...
        nvgStrokeColor(ctx, Color(m_theme->m_window_header_sep_bot - Vector4f(m_fade)));
        nvgStroke(ctx);
        ...
        nvgFontBlur(ctx, 2);
        nvgFillColor(ctx, Color(m_theme->m_drop_shadow - Vector4f(m_fade)));
        nvgText(ctx, m_pos.x() + m_size.x() / 2,
                m_pos.y() + hh / 2, m_title.c_str(), nullptr);

        nvgFontBlur(ctx, 0);
        nvgFillColor(ctx, m_focused ? Color(m_theme->m_window_title_focused - Vector4f(m_fade))
                                    : Color(m_theme->m_window_title_unfocused - Vector4f(m_fade)));
...
void Window::dispose() {
    ...
    m_fade = Color();
    m_enabled = false;
    Color closing = this->m_theme->m_window_fill_unfocused;
    printf("Closing colors: %f %f %f %f\n", closing[0], closing[1], closing[2], closing[3]);
}

void Window::dispose_now() {
    Widget *widget = this;
    while (widget->parent())
...
bool Window::mouse_drag_event(const Vector2i &, const Vector2i &rel,
    ...
    if (m_drag && (button & (1 << GLFW_MOUSE_BUTTON_1)) != 0) {
        m_pos += rel;
        m_pos = max(m_pos, Vector2i(0,25));
        m_pos = min(m_pos, parent()->size() - Vector2i(25,25));
        return true;
    }
    return false;
}

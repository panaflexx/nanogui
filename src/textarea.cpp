...
void TextArea::add_text(const std::string &text) {
    size_t lastBlock = m_blocks.size();

    if(lastBlock > 0)
        m_blocks[lastBlock-1].text += text;
}
        ...
        if (key == GLFW_KEY_C && modifiers == SYSTEM_COMMAND_MOD && action == GLFW_PRESS &&
            ...
            if (key == GLFW_KEY_DELETE) {
                ...
                for(size_t i = 0; i < m_blocks.size(); i++) {
                    text += "{" + std::to_string(i) + ":" + m_blocks[i].text + "}";
                    }
...
bool TextArea::keyboard_character_event(unsigned int codepoint) {
    if (focused()) {
        std::string convert(1, codepoint);

        printf("keyboard_character_event: code=%d\n", codepoint);
        add_text(convert);
